...
/* SN - included language tag in the Description and TextDefinition tables. Changed release to US edition. Commented out tables that were not in Snapshot release */

SET SCHEMA 'snomedct';

DO $$
DECLARE
  folder TEXT := 'D:/PostgreSQL/13/files/SnomedCT_US_20240901_trimmed'; -- Change the root directory based on your file location
  type TEXT := 'Snapshot'; -- Change between Full, Delta, Snapshot.
  release TEXT := 'US1000124_20240901'; -- Change between each release
  suffix TEXT := '_s'; -- Suffix of the database table. _f stands for full, _d stands for delta, _s stands for snapshot
  descLanguage TEXT := 'en'; -- Description language code
BEGIN
  ...
  EXECUTE 'COPY description' || suffix || '(id, effectivetime, active, moduleid, conceptid, languagecode, typeid, term, casesignificanceid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_Description_' || type || '-' || descLanguage || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'' , DELIMITER ''	'', QUOTE E''\b'')';

  EXECUTE 'TRUNCATE TABLE textdefinition' || suffix;
  EXECUTE 'COPY description' || suffix || '(id, effectivetime, active, moduleid, conceptid, languagecode, typeid, term, casesignificanceid) FROM '''
        || folder || '/' || type || '/Terminology/sct2_TextDefinition_' || type || '-' || descLanguage || '_' || release || '.txt'' WITH (FORMAT csv, HEADER true, ENCODING ''UTF8'', DELIMITER ''	'', QUOTE E''\b'')';
